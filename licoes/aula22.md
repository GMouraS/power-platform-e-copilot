...
- **Gatilho "Executar um fluxo do Copilot Studio":** Usado novamente no fluxo do Power Automate para receber o input do CEP do agente.
...
## 🛠 Exercícios ou Atividades Práticas
 Crie um fluxo do Power Automate que realize uma consulta a uma API pública utilizando o conector HTTP premium. Integre este fluxo a um tópico no Copilot Studio e teste sua execução.
 ...
 **Parte 1: Criar o Fluxo do Power Automate (Ação) com Conector HTTP**
 ...
 7.  Configurar a ação HTTP:
     ...
     *   Em "URI", construir a URL concatenando o endpoint base com o input do gatilho `CEP` (ex: `https://brasilapi.com.br/api/cep/v1/@{triggerBody()['text']}`). A expressão para o input de texto do gatilho é `triggerBody()['text']` se o input se chamar 'text', ou use o conteúdo dinâmico. Se o input for `CEP`, seria `triggerBody()['CEP']`.
 ...
 **Parte 2: Integrar o Fluxo no Tópico do Copilot Studio**
 ...
 2.  (Opcional) Criar um **Novo Tópico em branco** para a consulta de CEP, ou editar o tópico anterior (Certifique-se de que não haja conflito de gatilhos com o tópico anterior). Se usar o tópico anterior, **desabilitar** o tópico que fazia a requisição HTTP direta.
 ...
 **Dificuldades encontradas ou insights obtidos:**
 - A sintaxe para referenciar o input do gatilho na URL da ação HTTP no Power Automate pode ser um pouco confusa inicialmente (`triggerBody()['NomeDoInput']`).
 - Gerar e usar corretamente o esquema JSON na ação "Analisar JSON".
 - Mapear corretamente as saídas analisadas para as saídas do fluxo "Responder ao Copilot Studio".
 - Garantir que o tópico anterior (se existir) com gatilhos semelhantes está desabilitado para evitar confusão no teste.
 - O histórico de execução do fluxo no Power Automate é essencial para depurar problemas no fluxo em si.
...
- Tratamento de erros em chamadas HTTP no Power Automate (usando blocos Try/Catch/Finally ou configurações de "run after").
- Gerenciar autenticação de API mais complexa (OAuth, chaves em cabeçalhos/corpo) dentro do Power Automate.
- Diferenças e cenários ideais para usar a requisição HTTP direta no Copilot Studio vs. o conector HTTP no Power Automate.
- Licenciamento detalhado do conector HTTP premium.
